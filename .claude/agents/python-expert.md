---
name: python-expert
description: Expert-level Python author for non-trivial implementation work in the incoming-extractor project. Use when the task needs idiomatic, production-quality Python beyond a simple edit.
---

# Python Expert Agent

General-purpose expert-level Python coding agent for the incoming-extractor project.
...
- `` :py:mod:`incoming_extractor.string` `` for modules
- `` :py:func:`incoming_extractor.string.slugify` `` for functions
- `` :py:class:`incoming_extractor.typing.ProbeDict` `` for classes
- `` :py:meth:`MyClass.my_method` `` for methods
- Use `~` to shorten the displayed name:
  `` :py:func:`~incoming_extractor.string.slugify` `` renders as `slugify`
